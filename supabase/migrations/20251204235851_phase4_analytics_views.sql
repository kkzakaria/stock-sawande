...
CREATE OR REPLACE VIEW cashier_performance_summary AS
SELECT
  s.cashier_id,
...
-- Index for store-based sales queries
...
ON sales(payment_method) WHERE status = 'completed';

-- Index for cashier performance queries
CREATE INDEX IF NOT EXISTS idx_sales_cashier_created_at
ON sales(cashier_id, created_at);

-- Index for inventory stock alerts (quantity only - min_stock_level is in product_templates)
CREATE INDEX IF NOT EXISTS idx_inventory_quantity
ON product_inventory(quantity);

-- Index for product min_stock_level
CREATE INDEX IF NOT EXISTS idx_product_templates_min_stock
ON product_templates(min_stock_level);
...
GRANT EXECUTE ON FUNCTION get_payment_breakdown TO authenticated;
GRANT EXECUTE ON FUNCTION get_inventory_report TO authenticated;
GRANT EXECUTE ON FUNCTION get_store_comparison TO authenticated;
GRANT EXECUTE ON FUNCTION get_cashier_performance TO authenticated;

-- Grant select permissions on views
GRANT SELECT ON daily_sales_summary TO authenticated;
GRANT SELECT ON payment_method_summary TO authenticated;
GRANT SELECT ON top_products_summary TO authenticated;
GRANT SELECT ON cashier_performance_summary TO authenticated;
