...
// ============================================
// POS-specific proforma creation
// ============================================

const posProformaItemSchema = z.object({
  product_id: z.string().uuid('Invalid product ID'),
  name: z.string(), // For receipt display
  sku: z.string(),  // For receipt display
  quantity: z.number().int().positive('Quantity must be positive'),
  unit_price: z.number().nonnegative('Unit price must be non-negative'),
  discount: z.number().nonnegative('Discount must be non-negative').optional().default(0),
})

const createPOSProformaSchema = z.object({
  store_id: z.string().uuid('Invalid store ID'),
  customer_id: z.string().uuid('Invalid customer ID').optional().nullable(),
  items: z.array(posProformaItemSchema).min(1, 'At least one item is required'),
...
export interface POSProformaResult {
  id: string
  proforma_number: string
  created_at: string
  valid_until: string
...
/**
 * Create a proforma directly from POS with "sent" status
 * Returns complete data for receipt display
 */
export async function createPOSProforma(
  input: z.infer<typeof createPOSProformaSchema>
): Promise<ActionResult<POSProformaResult>> {
  try {
    // Validate input
    const validated = createPOSProformaSchema.parse(input)
    ...
    // Create proforma with "sent" status
    const { data: proforma, error: proformaError } = await supabase
      .from('proformas')
      .insert({
        store_id: validated.store_id,
      ...
      .select('id, proforma_number, created_at, valid_until, subtotal, tax, discount, total, status')
      .single()
    ...
    // Return complete data for receipt
    return {
      success: true,
      data: {
        id: proforma.id,
        ...
        valid_until: proforma.valid_until || validUntil.toISOString(),
        subtotal: proforma.subtotal,
        tax: proforma.tax,
        discount: proforma.discount || 0,
        total: proforma.total,
    ...
    console.error('Error in createPOSProforma:', error)
    return { success: false, error: 'Failed to create proforma' }
  }
}
